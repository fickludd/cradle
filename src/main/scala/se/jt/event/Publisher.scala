...
/*
 * This is shamelessly stolen from scala.swing
 */
...
trait Publisher extends Reactor {
  ...
  private[event] def subscribe(listener: Reaction) { listeners += listener }
  private[event] def unsubscribe(listener: Reaction) { listeners -= listener }
...
private[event] trait SingleRefCollection[+A <: AnyRef] extends Iterable[A] { self =>
...
private[event] class StrongReference[+T <: AnyRef](value: T) extends Reference[T] {
...
private[event] abstract class RefSet[A <: AnyRef] extends mutable.Set[A] with SingleRefCollection[A] { self =>
